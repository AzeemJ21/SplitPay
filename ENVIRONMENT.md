...
| `STORE_URL` | Your **store** origin only (e.g. `https://splitpay-store.onrender.com`) — must match the browser address bar on checkout; used for CORS on `/api/users/verify-code` and `/api/split-payment` |
...
1. **Dashboard (this service)** on Render:
   ...
   - `STORE_URL` = your deployed **store** URL (scheme + host, no path, no trailing slash).
